...
class MultiSelectContainer<T> extends StatefulWidget {
  const MultiSelectContainer({
    ...
    this.itemsChild = const MultiSelectCardinitialChild(),
  ...
  final MultiSelectChild itemsChild;
  ...
  // final void Function(List<T> selectedItems, T selectedItem) seletedSome;
...
class _SimpleMultiSelectContainerState<T>
  ...
  void initState() {
    ...
    if (widget.controller != null) {
      ...
      widget.controller!.selectSome = _seletedSome;
      widget.controller!.refreshUI = _refreshui;
    }
    super.initState();
  }
  ...
  List<T> _seletedSome(String list) {
    _selectedItems.clear();
    _selectedItems.clear();
    _selectedItems.addAll(_items.where((i) => i.value == list).toList());
    setState(() {});
    return _getValues();
  }
  ...
  Widget? getChild(
      MultiSelectNormalChild itemChild,
      MultiSelectChild commonItemsChild,
      bool isSelected,
      bool enabled,
      BuildContext context) {
    const simpleCardChild = MultiSelectCardinitialChild();

    final child = isSelected
        ? itemChild.seletedChild ??
            commonItemsChild.seletedChild ??
            simpleCardChild.getSelectedChild(context)
        ////
        : itemChild.child ??
            commonItemsChild.child ??
            simpleCardChild.getChild(context);
    return child;
  }
                ...
                children: [
                  ...
                  AnimatedDefaultTextStyle(
                    ...
                    style: getTextStyle(item.textStyles, widget.textStyles,
                        isSelected, item.enabled, context),
                    child: (isSelected ? item.seletedChild??item.child : item.child) ??
                        Text(
                          item.label!,
                        ),
                  ),
